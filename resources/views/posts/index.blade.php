        ...
        <div class="card-body p-3">
                ...
                <div>
                    <span class="me-2">
                        <a href="#" data-bs-toggle="modal" data-bs-target="#likesModal{{ $post->id }}" class="text-decoration-none">
                            {{ $post->likes->count() }} <i class="fas fa-thumbs-up text-primary"></i>
                        </a>
                    </span>
            ...
            <div class="d-flex justify-content-around mb-3">
                @if ($post->isLikedBy(auth()->user()))
                    <form action="{{ route('post.unlike', $post->id) }}" method="POST" class="me-2">
                        @csrf
                        <button type="submit" class="btn btn-light btn-block text-primary">
                            <i class="fas fa-thumbs-down"></i> Unlike
                        </button>
                    </form>
                @else
                    <form action="{{ route('post.like', $post->id) }}" method="POST" class="me-2">
                        @csrf
                        <button type="submit" class="btn btn-light btn-block text-primary">
